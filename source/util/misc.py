...
class NestedTensor(object):
    ...
    def to_img_list(self):
        if self.tensors.dim() == 3:
            ...
            for i in range(self.tensors.shape[0]):
                ...
                mask = self.mask[i]
                res.append(self.to_img_list_single(tensor, mask))
            return res
    ...
    @property
    def shape(self):
        return {"tensors": self.tensors.shape, "mask": self.mask.shape} 
...
def _onnx_nested_tensor_from_tensor_list(tensor_list: List[Tensor]) -> NestedTensor:
    ...
    for i in range(tensor_list[0].dim()):
        max_size_i = torch.max(
            torch.stack([img.shape[i] for img in tensor_list]).to(torch.int64)
        ).to(torch.int64)
    ...
    for img in tensor_list:
        ...
        m = torch.zeros(img[0], dtype=torch.int, device=img.device)
