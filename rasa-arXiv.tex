...
%, thus enabling better representation learning.
...
%Beyond that, we encourage the representation to have the ability to be aware of the sensitive transformation by SA (\emph{i.e.}, learning to detect the replaced words), thus promoting the representation's robustness.
...
%Experiments demonstrate that RaSa contributes to outstanding performance with \textbf{11.26\%} and \textbf{6.26\%} improvements compared to SOTA at Rank@1 on CUHK-PEDES and ICFG-PEDES datasets, respectively.
...
% \centering
% \vspace{0.5cm}
% \includegraphics[width=0.95\linewidth]{resources/figure1-camera_ready-v2.png}
% \caption{Comparison between the proposed method and other methods. Empty circle simulates feature space. The noise interference in the weak positive pair is highlighted in red. Image flipping is used as the insensitive transformation, while word replacement as the sensitive transformation in which the replaced words are in bold.}
% \label{fig1}
% \end{figure}
...
% As shown in Figure~\ref{fig1}, the images in (a) and (b) depict the same person at different times, while the text is an exact description of the image in (a).
% As a result, the image and text in (b) form a weak positive pair, in which there exists the local correspondence (\emph{``gray shoes"} in the text corresponding to a particular region in the image) and the noise interference (\emph{``white t-shirt"} and \emph{``blue shorts"} corresponding to non-existent objects in the image).
...
%Towards this end, we propose a Positive Relation Detection (PRD) pretext task to distinguish strong and weak positive pairs.
%Grounded on the common Image-Text Matching (ITM) as the \emph{de facto} optimization objective, PRD is appended to ITM.
%ITM emphasizes the consistency between strong and weak positive pairs, while PRD highlights their difference.
%As a result, PRD serves as the regularization of ITM, effectively preventing ITM from overfitting caused by the noise.
%The combination of ITM and PRD provides a Relation-Aware learning (RA).
%The model armed with RA can not only learn valuable information from weak positive pairs by ITM but also alleviate noise interference from them by PRD, reaching a trade-off.

%Towards this end, we propose a Relation-Aware learning (RA) task.
%Grounded on the common Image-Text Matching (ITM) as the \emph{de facto} optimization objective, we develop a probabilistic ITM ($p$-ITM) with a probabilistic inputting of strong and weak positive pairs, where PRD is appended to $p$-ITM to detect the positive relation.
%In essence, $p$-ITM emphasizes the consistency between strong and weak positive pairs, whereas PRD highlights their difference.
%As a result, PRD serves as the regularization of $p$-ITM.
%The model armed with RA can not only learn valuable information from weak positive pairs by $p$-ITM but also alleviate noise interference from them by PRD, reaching a trade-off.
...
%Learning feature representations with data augmentation is a general practice for improving the robustness of representation in the existing methods  \cite{caron2020unsupervised,chen2021exploring}.
%They aim to learn invariant representations under a set of manually chosen transformations (also called \emph{insensitive} transformations in this context).
...
%As shown in Figure~\ref{fig1}, the pair in (c) is an insensitive transformation of the pair in (b) with the image flipping operation, and the feature representations of pairs in (b) and (c) are learned to be closer.
...
%For this, we propose to adopt the word replacement as the sensitive transformation and develop a Momentum-based Replaced Token Detection (MRTD) pretext task to detect whether a token is from the original textual description or the replacement, as shown in Figure~\ref{fig1} (d).
...
% With these in mind, the word replacement is obtained via Masked Language Modeling (MLM), which utilizes the image and the contextual tokens in the text to predict the masked tokens.
...
% Moreover, we adopt the result of MLM from the momentum model that is a slow-moving average of the online model. 
% The momentum model can learn more stable representations than the current online model \cite{grill2020bootstrap}, hence generating more confusing words.
...
%However, it is unreasonable to directly adopt the vanilla ITM in text-based person search, since there exist too many weak positive pairs in text-based person search, which are usually accompanied with noise interference.
...
%It is worth noting that the ITM loss in Eq.~\ref{equ1} is computed by inputting the weak positive pair with a small probability of $p^w$ and the strong positive pair with the probability of $1-p^w$.
%In a word, we only retain a small proportion of weak positive pairs in ITM.
%For one thing, the weak positive pairs contain certain valuable alignment information that can facilitate representation learning. Hence, we keep the weak pairs in computing Eq.~\ref{equ1} to a certain extent.
%For another thing, there also exists noise interference in the weak positive pair, which hampers the representation learning. Therefore, we weaken the effect of the weak pairs on computing Eq.~\ref{equ1} by introducing the probabilistic inputting. 
...
% \begin{equation}
%     L_{prd} = \mathbb{E}_{p(I^p, T^p)}{\rm H}(y^{prd}, \phi^{prd}(I^p, T^p)), \label{equ2}
% \end{equation}
\begin{equation}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{resources/figure3-camera_ready.png} % Reduce the figure size so that it is slightly narrower than the column. Don't use precise values for figure width. This setup will avoid overfull boxes.
\caption{Illustration of $m$-RTD. It aims to detect whether a token is from the original textual description or the replacement with the aid of the information of the contextual tokens and the paired image. The text with word replacement is obtained by the result of the Masked Language Modeling (MLM) from the momentum model.}
\label{fig3}
\end{figure}
...
%For the text-based person search task, we perform the cross-modal CL task with an image-to-text loss $L_{nce}^{i2t}(v'_{cls}, \hat{t}'_{cls}, \hat{Q}_t)$ and a text-to-image loss $L_{nce}^{t2i}(t'_{cls}, \hat{v}'_{cls}, \hat{Q}_v)$.
%Meanwhile, we also perform the intra-modal CL task with $L_{nce}^{i2i}(v'_{cls}, \hat{v}'_{cls}, \hat{Q}_v)$ in the visual modality and $L_{nce}^{t2t}(t'_{cls}, \hat{t}'_{cls}, \hat{Q}_t)$ in the textual modality.

%Taken together, we define the overall loss for CL as:
%\begin{equation}
%    L_{cl} = (L_{nce}^{i2t} + L_{nce}^{t2i} + L_{nce}^{i2i} + L_{nce}^{t2t})\ /\ 4. \label{equ8}
%\end{equation}
...
\multirow{11}{*}{\rotatebox{90}{w/o VLP}}      & GNA-RNN \cite{li2017person}      & 19.05     & -      & 53.64      & -      \\
                                               ...
                                               %& SRCF \cite{suo2022simple}        & 64.04     & 82.99      & 88.81      & -      \\
...
\label{Baselines}
%Most existing works utilize external knowledge to facilitate learning and initialize their backbones by single-modality pretraining (\emph{e.g.}, ResNet~\cite{he2016deep} or ViT~\cite{dosovitskiy2020image} as the visual backbone and BERT~\cite{devlin2018bert} as the textual backbone), which are pretrained on unaligned images and texts separately without cross-modal interaction. 
%Recently, some works~\cite{shu2022see,yan2022clip} have shown the remarkable success of vision-language pretraining (VLP) on text-based person search.
%As shown in Table~\ref{table1}, methods with VLP can achieve better performance than those without VLP on the whole.
%Hence, we conduct RA and SA on three VLP baselines: ALBEF~\cite{NEURIPS2021_50525975}, TCL~\cite{yang2022vision} and CLIP~\cite{radford2021learning}.
%By default, we initialize the model parameters of RaSa by ALBEF~\cite{NEURIPS2021_50525975}. 
%\emph{More details and experiments on other baselines can be found in Appendix.}

%\textbf{ALBEF} is pretrained on $14$M image-text pairs where ITC and ITM are optimized following the image-text retrieval setting. 
%Since the sentences are longer in text-based person search, we modify the maximum length of the input texts as $50$.
%Besides, we only use horizontal flipping as data augmentation. 
%We denote the modified version as $\rm baseline_{mod}$ to distinguish with the vanilla baseline.
...
%Compared with SAF\footnote{We use the released code of SAF to reproduce the mAP on CUHK-PEDES and all results on ICFG-PEDES.}~\cite{li2022learning}, RaSa outperforms it by $10.77$\% and $8.53$\% in terms of mAP on the two datasets.
%DeBarl still outperforms other methods by a large superiority. Compared to SAF \cite{li2022learning}, our DeBarl brings an absolute boost of 10.42\% on R@1. 
The comparison clearly demonstrates the effectiveness of RaSa in text-based person search.


\subsection{Ablation Study}
%To thoroughly study the contribution of each optimization objective and the effectiveness of other variants of RA and SA, we conduct a series of ablation experiments on CUHK-PEDES as shown in Table~\ref{table4}.
%From NO.2, a modified ALBEF~\cite{NEURIPS2021_50525975} provides a strong baseline, which demonstrates the remarkable effectiveness of VLP on text-based person search again.
...
%In order to adequately verify the effectiveness and contribution of each optimization objective in RaSa, we show the results with different optimization objectives in Table~\ref{table3}. 
...
\centering
\includegraphics[width=0.95\textwidth, height=0.17\textwidth]{resources/figure4.png} % Reduce the figure size so that it is slightly narrower than the column. Don't use precise values for figure width. This setup will avoid overfull boxes.
\caption{The impact of the hyper-parameters at R@1 on CUHK-PEDES. $p^w$ denotes the probability of inputting weak positive pairs in RA. $p^m$ means the masking ratio of the tokens in a text in SA. $\lambda_1$, $\lambda_2$ and $\lambda_3$ are the loss weights.}
\label{fig4}
\end{figure*}
...
%We show the ablation results of RaSa with different settings in RA from NO.4 to NO.7 in Table~\ref{table4}.

%On the basis of CL for representation learning, we add ITM$_{all}$ that learns from all positive pairs without the probabilistic inputting and brings a noticeable improvement in terms of all metrics.
The vanilla ITM learns from all positive pairs without the probabilistic inputting.
%However, it neglects the noise inference from the weak positive pairs, which results in overfitting learning.
...
%Not surprisingly at all, compared with the vanilla ITM, $p$-ITM is less disturbed by the noise; compared with $s$-ITM, although it takes the weak supervision into consideration, the noise from the retained weak positives still hampers the performance.
...
%On the whole, RaSa with ITM and PRD presents more competitive results than one with ITM$_{all}$ and PRD.
...
%We present the ablation results of SA with various settings from NO.8 to NO.11 in Table~\ref{table4}.

Based on CL and RA, adding a single MLM without the replacement detection task brings a slight boost of $0.61$\% at R@1.
%Furthermore, we introduce the detection task and compare the results from three different generators.
...
%Moreover, we continue to perform extended experiments (\emph{detailed in Appendix}) on two coarse-grained image-text retrieval datasets (Flickr30K~\cite{plummer2015flickr30k} and COCO~\cite{lin2014microsoft}) and other fine-grained datasets (CUB~\cite{reed2016learning} and Flowers~\cite{reed2016learning}).
%In order to verify the generalization ability of RaSa, we conduct a series of domain generalization experiments (\emph{presented in Appendix}) following LGUR~\cite{shao2022learning}.

%The whole results clearly demonstrate the effectiveness of RaSa.
%And the visualization in Appendix also vividly presents the excellent retrieval ability.
...
%BEIT-3~\cite{wang2022image}     & 98.00 & 100.00 & 100.00 & 90.30 & 98.70 & 99.50   & 84.80 & 96.50 & 98.30  & 67.20 & 87.70 & 92.80   \\
\hline
...
%More importantly, each image in Flickr30K and COCO is a unique class, while images in text-based person search are deemed as the same class if they are corresponding to the same person.
%Therefore, all of the positive image-text pairs in Flickr30K and COCO belong to the strong positive type.
%The proposed RA, which aims at differentiating between strong and weak positive pairs, no longer applies to the experiments in Flickr30K and COCO.
...
%Since there exists none of weak positive pairs, we only perform SA which focuses on the sensitive transformation (\emph{i.e.}, MLM-based token replacement) and use the vanilla ITM to replace the proposed RA which mainly aims at the positive relation.

%We compare RaSa with the baseline ALBEF~\cite{NEURIPS2021_50525975}\footnote{We report the results reproduced with the released code of ALBEF, where the batch size is set as same as the introduction in Section~\ref{Implementation Details} of the Appendix for a fair comparison.} as shown in Table~\ref{table1}.
%For the coarse-grained retrieval, RaSa with only SA still brings consistent improvement in terms of all metrics.
...
%During inference, existing methods usually first average text embeddings of each class, and rank all test images according to the similarity with the averaged class embeddings.
%Since RaSa is a one-stream model where the final outputs is cross-modal embeddings rather than text embeddings, we can not average them as the class embeddings.
%Therefore, we first calculate the predicted matching probability of every image-text pair from $p$-ITM without the top-$128$ selecting and then average the matching probabilities of each text class. The averaged matching probabilities is utilized to implement the ranking.
...
%TCL additionally introduces the intra-modal self-supervised learning and maximizes the mutual information between local structures of image/text and their global summary.
%$4$M image-text pairs are adopted to pretrain the model.
...
%Unlike other baselines which have a cross-modal encoder to perform deep fusion, CLIP is a two-stream model where cross-modal fusion only depends on contrastive learning loss.
%However, both RA and SA work on multi-modal features.
...
%Notably, no matter whether TCL or CLIP is adopted as the backbone, RaSa always brings consistent improvements in terms of all metrics.
%Specifically, RaSa$\rm _{TCL}$ and RaSa$\rm _{CLIP}$ outperform their backbones by $15.63$\% and $14.55$\% at R@1.
%The promotion significantly demonstrates the effectiveness of RaSa.
...
%We conjecture that the additionally appended one-layer transformer block is totally randomly initialized, rendering the model inclined to be trapped in the local minimum.
%Although the backbone CLIP is pretrained to optimize the image encoder and the text encoder, lack of cross-modal deep fusion makes the model difficult to understand more fine-grained details, which are the core of text-based person search.
%Besides, the lack of cross-modal deep fusion in the backbone CLIP makes the model difficult to capture fine-grained details, which are the core of text-based person search.
%The observation also enlightens us to design a more effective cross-modal fusion strategy in future work.


\begin{figure}[t]
\centering
\includegraphics[width=1\linewidth, height=1.4\linewidth]{resources/Figure1_Appendix.png} % Reduce the figure size so that it is slightly narrower than the column. Don't use precise values for figure width. This setup will avoid overfull boxes.
\caption{Visualization of top-10 retrieval results on CUHK-PEDES.
